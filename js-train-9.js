...
function getPositiveArrayValues(arr) {
    ...
    const valueArray = Array.from(arr.values());
    const positiveValueArray = valueArray.filter((value) => value > 0);

    return positiveValueArray;
}
...
function sumAndPrint(arr) {
    ...
    const sum = arr.reduce((accum, currentValue) => accum + currentValue, 0);
...
function flattenSquareAndFilter(arr, threshold) {
    ...
    const squaredArr = arrFlat.map((num) => num ** 2);
    const filterArr = squaredArr.filter((num) => num > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
    ...
    lastElements.forEach((num) => console.log(num));
...
function expandArrayByN(arr, n) {
    ...
    const expandedArr = arr.flatMap((num) => Array(n).fill(num));
    return expandedArr;
}
