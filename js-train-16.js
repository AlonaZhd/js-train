...
// Виведе Ім'я студента: John, Ім'я студента: Mike
...
function logArrayElements(arr) {
    ...
    arr.forEach((element, index) => {
        console.log(`Елемент ${index}: ${element}`); // Виведення поточного елемента масиву та його індексу
    });
}
...
compareArrays([1, 2, 5, 3, 4, 5], [1, 2, 5, 3, 4]);
...
function validateUserInput(userInput) {
    ...
    if (userInput.password.length < 8) {
        // console.assert("Попередження: пароль має бути довшим за 8 символів!"); не працює на моєму ноуті
...
function calculateTotalPrice(products) {
    ...
    const total = products.reduce((num, product) => num + product.price, 0);
